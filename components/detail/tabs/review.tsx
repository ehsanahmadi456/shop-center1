...
import { IProducts, Comments } from '@/models/cart';
...
const Review = ({ data }: IProp) => {
  ...
  return (
    <div className="mt-11">
      ...
      {data?.comments.filter((comment, idx) => idx > (page * 3) - 3 && idx <= page * 3).map((item: Comments, idx: number) =>
        <div className={`flex flex-row items-center justify-start ${idx !== 0 && 'mt-10'}`} key={item.id}>
          <div className="flex flex-col justify-center items-start w-[54%]">
            <div className="flex flex-row items-center justify-start">
              <Image src='/images/detail/man.jpg' alt="profile" width={24} height={24} className="rounded-full" />
          ...
          <div className="w-[15%] ml-[1%]">
            <Image src={data.images[0]} width={150} height={250} alt="product" className="!w-full !h-fit" />
          </div>
          <div className='flex flex-row items-center w-[17%] ml-[10%]'>
            <div className='flex flex-row items-center'>
              ...
              <Body2 className="text-dark-500 ml-2.5">Helpful ({item.helpfull})</Body2>
            </div>
      ...
      {data &&
        <Pagination
          ...
          count={Math.ceil(data.comments.length / 3)}
          color="primary"
          size="large"
          variant="outlined"
          shape="rounded"
