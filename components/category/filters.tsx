...
interface IProps {
  ...
  displayValue: number,
  listSleeve: {
    Sleeveless: boolean, 'Short Sleeve': boolean, 'Long Sleeve': boolean, 'Half Sleeve': boolean
  }
  listSize: {
    FreeSize: boolean, XS: boolean, S: boolean, M: boolean, L: boolean, XL: boolean
  },
  listBrand: {
    'H&M': boolean, Adidas: boolean, Defacto: boolean, Fendi: boolean,
    Prada: boolean, Nike: boolean, MANGO: boolean
  },
  filterProducts: () => void,
  changeCheckbox: (event: React.ChangeEvent<HTMLInputElement>, type: string) => void,
  changeProgress: (event: Event, newValue: number | number[]) => void
}

const Filters = ({ progress, listBrand, displayValue, listSleeve, listSize, changeCheckbox, changeProgress, filterProducts }: IProps) => {
  return (
